...
- A compile reports its diagnostics in one order, `issues.report_order`: by line (none last), column (none last), severity (errors first), then code; ties keep the order they were emitted in. `compile_source` sorts the list once, after pragmas apply, and `report()` and `to_dict()` list in the same order. `validate()` alone returns issues in check order, since most get their line from the compiler afterwards. The agent's revision prompt (`agent.render_feedback`) deliberately lists whole-plan notes (no line) first: the compile's plan-level diagnostics in report order, then the agent's own notes in the order it appended them. The line-anchored diagnostics follow in report order.
