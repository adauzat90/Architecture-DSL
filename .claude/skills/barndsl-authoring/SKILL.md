...
plan and adds a structural takeoff. Info nudges: `BAY_WIDE` (bay > ~12 ft o.c.),
`POST_OBSTRUCT` (an interior post stranded in open floor — align a partition to
it). It's a layout aid, not an engineered design. `barndsl build FILE --frame`
frames a plan that has no `frame` line.
